...
async function getProductFromDB(category: string, urlId: string): Promise<Product> {
    const r = await fetch(`http://localhost:8082/product/${category}/${urlId}`)
    return r.json() as Promise<Product>
}

async function getProductsCategory(category: string): Promise<Product[]> {
    const r = await fetch(`http://localhost:8082/product/${category}`)
    return r.json() as Promise<Product[]>
}

export async function getProducts(req: Request, res: Response){
    const r = await fetch(`http://localhost:8082/product/${req.body.category}`)
